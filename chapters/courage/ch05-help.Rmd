...
The **Inline R** works in YAML when written exactly as above. Notice the backticks around the letter `r` and the `Sys.Date()` function and that it is wrapped in quotes \`"`r Sys.Date()`"\`. You can learn more about inline R in the 📝 Notebook Advanced Activity, but for now, you can copy and paste the above YAML section to add a dynamic date to your YAML header. Each time you **knit** the document, the date will update to the current date. 
...
![Figure 1. Screenshot of R markdown formating](/assets/images/ch05-help/Memo_Example_heading.png)
...
RStudio UI varies. Some of you will have a 🧶**Knit button**, others will use **Preview menu** drop.
...
![Figure 2. Screenshot of knit button](/assets/images/ch05-help/Memo_Example_knithtml3.png)
...
![Figure 3. Screenshot of Preview button](/assets/images/ch05-help/Memo_Example_knithtml.png)
...
![Figure 4. Screenshot of resulting HTML report](/assets/images/ch05-help/Memo_Example_knithtml2.png)
...
> 📏 **Rule:** Your notebook must create every object it uses in the notebook itself. Your Environment does not count. Knitting may reveal a code error you did not notice. That is useful information about the *first place* your workflow that needs attention.
...
To share (e.g., submit), 📤 upload the `.html` file. 💡 **Tip:** Check the HTML first to make sure it is correct and shows the correct formatting.

### Knit Errors

When you select to knit, the bottom left pane shows progress messages in the **Render tab** as R runs your notebook from top to bottom. 
...
🌳 For more troubleshooting R Markdown, refer to the [Nick Tierney’s “Common Problems with rmarkdown](https://rmd4sci.njtierney.com/common-problems-with-rmarkdown-and-some-solutions).

---

Next, you will shift from building the report to writing code inside it. 💡 **Tip:** Knit after any meaningful formatting or code change. It is easier to find an error when it was caused by your most recent edit.
...
To make the function do its job, you have to _call it_. A **function call** runs the stored code. To call a function, you type the function name followed by parentheses `()`. Inside the parentheses, you must provide any **required arguments**. For example, to calculate the mean of a numeric vector, you call the `mean()` function and provide the vector as an argument. The vector is a **required argument** because the function needs it to perform the calculation. 
...
`?[function_name]` opens the help file for that function. The **Files pane** usually contains several tabs, including the **Help tab**, which displays documentation for R functions.
...
The **Examples section** is often one of the most useful parts of a help page. Examples show working patterns. Your job is to adapt the pattern to your own object.
...
This Practice Space helps you check whether you can organize a notebook, read an R help page, use a function argument, and knit your work into an HTML report.

You will **not** create a new R Notebook in RStudio.

Instead, choose the best answer for each question. These questions help you check whether you can organize an R Notebook, read R help pages, use function arguments, and understand why knitting is an important reproducibility check.
